...
public class QuadHashTable<E>
{
    ...
    public QuadHashTable(int hashSize)
    {
    ...
    public int procPos(E item)
    {
        ...
        {
            while(begin < maxDifference)
            {
                if(table[pos] != null && !table[pos].removed && table[pos].getItem().equals(item))
                {
                    return pos;
                }
                pos = Math.abs(item.hashCode() % this.hashSize);
                pos = Math.abs((pos + counter ^ 2) % this.hashSize);
                counter++;
                begin++;
            }
        }
